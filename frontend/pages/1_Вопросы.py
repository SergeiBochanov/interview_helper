...
from api_client import get_question, submit_answer, APIError
from auth import ensure_user
from mock_api import DIRECTIONS, TOPICS, DIFFICULTIES
...
with col2:
    topic = st.selectbox("Тема", TOPICS, key="q_topic")
with col3:
...
if "current_question" in st.session_state:
    ...
    if "last_result" in st.session_state:
        ...
        # verdict бэкенд не присылает (см. interviewer.py) — тон вывода
        # вычисляем на своей стороне по числовому score
        if r["score"] >= 4:
